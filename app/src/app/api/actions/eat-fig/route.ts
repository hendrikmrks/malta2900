...
import { EAT_FIG_HUNGER_GAIN } from "@/lib/game";
...
export async function POST() {
  ...
  if (playerState.figCount <= 0) {
    return NextResponse.json({ errorCode: "NO_FIGS" }, { status: 409 });
  }

  await prisma.playerState.update({
    where: { id: playerState.id },
    data: {
      hunger: Math.min(100, playerState.hunger + EAT_FIG_HUNGER_GAIN),
      figCount: { decrement: 1 },
    },
  });
