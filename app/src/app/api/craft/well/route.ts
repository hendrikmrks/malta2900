...
import { WELL_STONE_COST, WELL_WOOD_COST, XP_CRAFT, levelForXp } from "@/lib/game";
...
export async function POST(request: Request) {
  ...
  if (playerState.hasWell) {
    return NextResponse.json({ errorCode: "WELL_ALREADY_BUILT" }, { status: 409 });
  }

  if (playerState.woodCount < WELL_WOOD_COST) {
    return NextResponse.json({ errorCode: "NOT_ENOUGH_WOOD" }, { status: 409 });
  }
  if (playerState.stoneCount < WELL_STONE_COST) {
  ...
  await prisma.playerState.update({
    ...
    data: {
      woodCount: { decrement: WELL_WOOD_COST },
      stoneCount: { decrement: WELL_STONE_COST },
      hasWell: true,
      wellX: Math.round(clamp(x, 0, 100)),
      wellY: Math.round(clamp(y, 0, 100)),
      xp,
      level: levelForXp(xp),
    },
  });
